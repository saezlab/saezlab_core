...
dependencies needed for running saezlab_core (`[tool.poetry.dependencies]` with the
...
developing saezlab_core using test-driven development. Please make sure that you
...
If you are brand new to saezlab_core or open-source development, we recommend
...
saezlab_core is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of saezlab_core

You will need your own fork of saezlab_corein order to eventually open a Pull
Request. Go to the saezlab_core project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/saezlab_core.git
cd saezlab_core
git remote add upstream https://github.com/saezlab_core/saezlab_core.git
git fetch upstream
```

This creates the directory `saezlab_core` and connects your repository to the
upstream (main project) *saezlab_core* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of saezlab_core
repository. First ensure it's up-to-date with the main saezlab_core repository.
...
to create a branch called `my-new-feature-for-saezlab_core`

```bash
git checkout -b my-new-feature-for-saezlab_core
```

This changes your working branch from `main` to the
`my-new-feature-for-saezlab_core` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *saezlab_core*. You
...
When making changes, follow these saezlab_core-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the saezlab_core project.
...
1. Write a descriptive title that includes prefixes. saezlab_core uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the saezlab_core `main` branch, run:

```shell

    git checkout my-new-feature-for-saezlab_core
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-saezlab_core
```
